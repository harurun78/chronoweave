...
Step 2:   ブランチ作成 & 実装
    ...
    ↓
Step 3:   検証 (npm run lint && npm run type-check && npm run test:run && npm run test:e2e && npm run build)
    ↓
...
```
runSubagent(agentName="reviewer", prompt="PR #<NUMBER> をレビューしてください。リポジトリは OWNER/chronoweave です。")
```
...
```sh
# レビューコメントを取得
GH_PAGER=cat gh api repos/OWNER/chronoweave/pulls/<NUMBER>/reviews
GH_PAGER=cat gh api repos/OWNER/chronoweave/pulls/<NUMBER>/comments
```
