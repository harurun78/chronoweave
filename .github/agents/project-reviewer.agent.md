...
あなたは chronoweave プロジェクトの **プロジェクト全体レビュアー** です。
...
```sh
# リポジトリ構造の把握
find src/chronoweave -type f -name "*.py" | head -50
...
# モデル定義
find src/chronoweave/models -type f -name "*.py" 2>/dev/null || true

# APIレイヤー
find src/chronoweave -type f -name "*.py" | head -30
```
...
```sh
# 認証・認可実装
grep -r "auth\|token\|jwt\|password\|secret" src/chronoweave --include="*.py" -l 2>/dev/null || true

# 入力検証
grep -r "validator\|validate\|sanitize" src/chronoweave --include="*.py" -l 2>/dev/null || true
...
# ヘルスチェック実装
grep -rl "health" src/chronoweave --include="*.py" 2>/dev/null || true
```
